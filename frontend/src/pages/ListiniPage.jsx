...
function DuplicaListinoModal({ listino, onConfirm, onCancel }) {
  ...
  const submit = async (e) => {
    e.preventDefault()
    if (!nome.trim()) { setErr('Il nome è obbligatorio'); return }
    setSaving(true)
    try {
  ...
  return (
    <Modal title="Duplica listino" onClose={onCancel}>
      <form onSubmit={submit} className="space-y-4">
        <p className="text-sm text-gray-600">
          Verrà creata una copia di <strong>{listino.nome}</strong> con tutte le sue voci.
        </p>
        {err && <p className="text-sm text-red-600">{err}</p>}
        <div>
          <label className="block text-xs font-medium text-gray-600 mb-1">Nome del nuovo listino *</label>
          <input
            type="text"
            value={nome}
...
export default function ListiniPage() {
  ...
  const [duplicateListino, setDuplicateListino] = useState(null)
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Modal: Duplica listino */}
      {duplicateListino && (
        <DuplicaListinoModal
          listino={duplicateListino}
          onConfirm={async (nome) => {
            await listiniApi.duplicate(duplicateListino.id, { nome })
            setDuplicateListino(null)
            load()
          }}
          onCancel={() => setDuplicateListino(null)}
        />
      )}
